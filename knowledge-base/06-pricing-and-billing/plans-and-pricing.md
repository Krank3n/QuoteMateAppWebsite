---
id: billing-plans-and-pricing
title: Plans and pricing
category: Pricing & Billing
audience: [customers, prospects]
last_updated: 2026-07-25
keywords: [pricing, plans, free, pro, cost, price, monthly, annual, how much, subscription]
question_examples:
  - "How much does QuoteMate cost?"
  - "What's the difference between Free and Pro?"
  - "Is there a free plan?"
  - "How much is the annual plan?"
  - "What do I get on Pro?"
---

# Plans and pricing

QuoteMate has a free plan you can use forever, and a Pro plan for tradies who want the lower Square fee, more payment options, and every feature. All prices are in AUD.

## The plans

| | Free | Pro Monthly | Pro Annual |
|---|---|---|---|
| **Price** | **$0 forever** | **$49/month** | **$328/year** (≈ $27.33/mo, save $260/yr) |
| Credit card to start | No | 14-day free trial | 14-day free trial |

> **Founding-member pricing:** $49/month and $328/year are founding-member prices, locked for life for early subscribers. Once the founding spots fill, the price for new members rises to $99/month and $658/year. The pricing page shows live availability — if no founding note appears there, the offer has closed.

One subscription covers iPhone, iPad, Android, and web — there's no per-device charge.

## What's on Free

- **Unlimited quotes and invoices**
- Live supplier pricing
- Cloud sync across your devices
- Professional PDF template (one)
- Customers pay online via Square (a 1.7% platform fee is added to the customer's bill)

## What Pro adds

- **Lower Square fee** — 1.0% online / 1.5% in-person, vs 1.7% on Free
- More payment options — **Bank transfer, PayID, BPAY, PayPal**
- **AI material and title generation**
- **All PDF templates**
- Your **business logo** on documents
- Invoice and payment tracking
- **Priority support**

## Free trial

New users get a **14-day Pro trial** that starts when you create your first quote, so you can try everything before deciding. See [Free trial: how it works](free-trial.md).

## Related

- [Free trial: how it works](free-trial.md)
- [Managing your subscription](managing-your-subscription.md)
- [Getting paid with Square (fees)](../03-invoicing-and-payments/getting-paid-with-square.md)
